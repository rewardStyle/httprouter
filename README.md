...
**No more server crashes:** You can set a [Panic handler](https://godoc.org/github.com/donutloop/httprouter#Router.PanicHandler) to deal with panics occurring during handling a HTTP request. The router then recovers and lets the `PanicHandler` log what happened and deliver a nice error page.
...
Of course you can also set **custom [`NotFound`](https://godoc.org/github.com/donutloop/httprouter#Router.NotFound) and  [`MethodNotAllowed`](https://godoc.org/github.com/donutloop/httprouter#Router.MethodNotAllowed) handlers** and [**serve static files**](https://godoc.org/github.com/donutloop/httprouter#Router.ServeFiles).

## Usage

This is just a quick introduction, view the [GoDoc](http://godoc.org/github.com/donutloop/httprouter) for details.
...
Every `*<num>` represents the memory address of a handler function (a pointer). If you follow a path trough the tree from the root to the leaf, you get the complete route path, e.g `\blog\:post\`, where `:post` is just a placeholder ([*parameter*](#named-parameters)) for an actual post name. Unlike hash-maps, a tree structure also allows us to use dynamic parts like the `:post` parameter, since we actually match against the routing patterns instead of just comparing hashes. [As benchmarks show](https://github.com/donutloop/go-http-routing-benchmark), this works very well and efficient.
...
func main() {
	...
	router := httprouter.New()
	router.GET("/", Index)
	router.Handler(http.MethodGet, "/protected/", BasicAuth(http.HandlerFunc(Protected), user, pass))
...
**NOTE: It might be required to set [`Router.HandleMethodNotAllowed`](https://godoc.org/github.com/donutloop/httprouter#Router.HandleMethodNotAllowed) to `false` to avoid problems.**

You can use another [`http.Handler`](https://golang.org/pkg/net/http/#Handler), for example another router, to handle requests which could not be matched by this router by using the [`Router.NotFound`](https://godoc.org/github.com/donutloop/httprouter#Router.NotFound) handler. This allows chaining.
